...
describe('UniversityAdministratorStaff Component', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    localStorage.clear();
  });
  ...
  test('renders "You don\'t have permission" message when role is not 2', () => {
  ...
  test('fetches project data on mount', async () => {
    localStorage.setItem('role', '2');
    api.getAllProjectsOfAllUnis.mockResolvedValueOnce([{ id: 1, name: 'Project 1' }]);
    render(<UniversityAdministratorStaff />);
    await waitFor(() => {
